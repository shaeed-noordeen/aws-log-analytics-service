...
This environment deploys the Log Analyzer service stack in the `prod` workspace. It assembles the reusable modules under `terraform_modules/` to provide networking, container registry, IAM roles, ECS service, ALB, and CloudFront distribution.
...
```bash
cd log_analyzer/app/terraform/envs/prod
terraform init
...
```bash
terraform -chdir=log_analyzer/app/terraform/envs/prod output -raw <name>
```
